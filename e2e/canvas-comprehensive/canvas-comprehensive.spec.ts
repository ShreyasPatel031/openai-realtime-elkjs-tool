...
test.describe('Comprehensive Canvas Test Suite', () => {
  ...
  test('Coordinate Round-Trip - drag then refresh preserves position', async ({ page }) => {
    ...
    // Position after refresh should match position after drag (within snap-to-grid tolerance of 16px)
    expect(Math.abs(afterRefreshPos.x - afterDragPos.x)).toBeLessThan(16);
    expect(Math.abs(afterRefreshPos.y - afterDragPos.y)).toBeLessThan(16);
  });
