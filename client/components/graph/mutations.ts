 ...
 */
export const groupNodes = (nodeIds: NodeID[], parentId: NodeID, groupId: NodeID, style: any, graph: RawGraph): RawGraph => {
  ...
  if (style) {
    ...
    groupNode.data = {
      ...
      label: groupId,
      style: resolvedStyle,
      isGroup: true // Mark as group node even when empty
    };
  } else {
  ...
  // Add the group regardless of whether it has children (allow empty groups)
