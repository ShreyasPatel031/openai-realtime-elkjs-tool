...
function generateIconLists(): IconLists {
  ...
  if (fs.existsSync(canvasDir)) {
    ...
    console.log(`📦 Found ${iconLists.generic.length} generic icons in canvas directory`);
...
`;
  ...
  console.log(`✅ Generated icon lists: ${outputFile}`);
  console.log(`📊 Total icons: ${iconLists.all.length}`);
  ...
  console.log(`🏷️  Provider-prefixed icons: ${allPrefixedIcons.length}`);
}
 ...
 */
function main(): void {
  console.log('🔍 Scanning organized icon directories...');
  
  const iconLists = generateIconLists();
  generateIconListsFile(iconLists);
  
  console.log('✨ Icon list generation complete!');
}
