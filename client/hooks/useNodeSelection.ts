...
export const useNodeSelection = ({ selectedTool, setNodes }: UseNodeSelectionProps) => {
  ...
  const onSelectionChange = useCallback(({ nodes: selectedNodesParam, edges: selectedEdgesParam }: { nodes: Node[]; edges: Edge[] }) => {
    ...
    // Group tool: allows selection (like select tool) so users can select nodes then group them
    ...
    // Normal selection handling for select and group tools
    // Both tools allow selection - group tool uses selection to create groups
